...
#   - teacher forcing OFF (rollout-based training)
#   - GPUs 2,3 (joint backbone training)
...
echo "Starting DiffusionDrive-AR step-corner v2048 (heading head + step-aware agent, NO teacher forcing)..."
...
echo "Teacher forcing: OFF (rollout-based)"
echo "Trunk freeze: OFF (backbone joint training)"
echo "GPUs: CUDA_VISIBLE_DEVICES=2,3"
...
python -m navsim.planning.script.run_training \
    ...
    +experiment_name=diffusiondrive_ar_step_corner_v2048_heading_stepagent_notf \
    ...
    output_dir=/data2/byounggun/diffusiondrive_ar_output/heading_stepagent_notf \
    wandb.enabled=true \
    wandb.project="diffusiondrive-ar" \
    wandb.name="diffusiondrive_ar_step_corner_v2048_heading_stepagent_notf"
