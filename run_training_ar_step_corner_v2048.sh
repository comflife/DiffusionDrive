#!/bin/bash
# Train DiffusionDrive-AR with pure discrete single-step corner tokens.
# Each token is [4, 2] box corners for one local action, decoded to
# [dx, dy, dtheta] and composed autoregressively for 8 steps.
...
echo "Starting DiffusionDrive-AR step-corner codebook training..."
...
python -m navsim.planning.script.run_training \
    ...
    +experiment_name=diffusiondrive_ar_step_corner_v2048 \
    ...
    wandb.name="diffusiondrive_ar_step_corner_v2048"
