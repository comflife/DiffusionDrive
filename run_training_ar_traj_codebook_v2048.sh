#!/bin/bash
# Train DiffusionDrive-AR with a pure discrete whole-trajectory codebook.
# The codebook token stores the full 8-step ego box-corner trajectory, so
# XY and heading are both decoded from the selected token.
...
echo "Starting DiffusionDrive-AR pure trajectory-codebook training..."
...
echo "Using trajectory codebook: /home/byounggun/DiffusionDrive/codebook_cache/navsim_kdisk_v2048_diffusiondrive/ego.npy"
echo "Decoder: one discrete token -> full 8-step XY + heading trajectory"
...
python -m navsim.planning.script.run_training \
    ...
    +experiment_name=diffusiondrive_ar_traj_codebook_v2048 \
    ...
    wandb.name="diffusiondrive_ar_traj_codebook_v2048"
