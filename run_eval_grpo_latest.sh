#!/bin/bash
# Eval script for GRPO Latest checkpoint (last.ckpt)
...
# Use latest checkpoint (last.ckpt)
GRPO_CKPT="/data2/byounggun/diffusiondrive_grpo_output_v2/checkpoints/last.ckpt"

if [ ! -f "$GRPO_CKPT" ]; then
    echo "ERROR: Checkpoint not found at $GRPO_CKPT"
    exit 1
fi

echo "=================================================="
echo "Evaluating GRPO Model (Latest - last.ckpt)"
echo "=================================================="
echo "Checkpoint: $GRPO_CKPT"
echo "Modified: $(stat -c %y "$GRPO_CKPT" 2>/dev/null || stat -f %Sm "$GRPO_CKPT" 2>/dev/null)"
echo "=================================================="
...
python3 -m navsim.planning.script.run_pdm_score \
    ...
    output_dir=/data2/byounggun/diffusiondrive_grpo_output_v2/eval_latest
