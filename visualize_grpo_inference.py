#!/usr/bin/env python3
"""
Visualization script for DiffusionDrive-GRPO model evaluation.

Loads the GRPO model, evaluates on selected scenes from NAVSIM,
and saves BEV + camera visualizations with PDM scores.

Usage:
    python visualize_grpo_inference.py [--gpu 0] [--num_scenes 20] [--mode worst]
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Visualize DiffusionDrive-GRPO inference results")
...
def load_model(ckpt_path: str, device: torch.device) -> TransfuserAgentAR:
    """Load TransfuserAgentAR model from checkpoint (GRPO uses same architecture as AR)."""
...
def plot_bev_with_trajectories(
    ...
    """
    Plot BEV with GRPO model and human trajectories, including PDM score text.
    """
    ...
    add_trajectory_to_bev_ax(ax, grpo_trajectory, TRAJECTORY_CONFIG["agent"])
    configure_bev_ax(ax)
    configure_ax(ax)
    ...
    score_text = (
        f"Token: {token}\n"
        f"GRPO  PDMS: {pdm_result.score:.4f}  "
    ...
    legend_elements = [
        Line2D([0], [0], color=TRAJECTORY_CONFIG["agent"]["line_color"], lw=2, label="GRPO Model"),
...
def plot_combined_view(
    ...
    add_trajectory_to_bev_ax(ax[1, 1], grpo_trajectory, TRAJECTORY_CONFIG["agent"])
...
def main():
    ...
    for idx, token in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            # Run GRPO model inference (on GPU)
            with torch.no_grad():
                grpo_trajectory = model.compute_trajectory(agent_input)

            # Get human trajectory
            human_trajectory = human_agent.compute_trajectory(agent_input, scene)

            # Compute PDM scores
            grpo_pdm = pdm_score(metric_cache, grpo_trajectory, simulator.proposal_sampling, simulator, scorer)
            human_pdm = pdm_score(metric_cache, human_trajectory, simulator.proposal_sampling, simulator, scorer)

            print(f"    GRPO  PDMS: {grpo_pdm.score:.4f} (NC={grpo_pdm.no_at_fault_collisions:.1f}, "
                  f"DAC={grpo_pdm.drivable_area_compliance:.1f}, EP={grpo_pdm.ego_progress:.3f})")
            ...
            fig_bev = plot_bev_with_trajectories(
                scene, grpo_trajectory, human_trajectory, grpo_pdm, human_pdm, token
            )
            ...
            results_summary.append({
                "idx": idx,
                "token": token,
                "grpo_score": grpo_pdm.score,
                "grpo_nc": grpo_pdm.no_at_fault_collisions,
                "grpo_dac": grpo_pdm.drivable_area_compliance,
                "grpo_ep": grpo_pdm.ego_progress,
                "grpo_ttc": grpo_pdm.time_to_collision_within_bound,
                "grpo_comfort": grpo_pdm.comfort,
