#!/bin/bash
# Train DiffusionDrive-AR — v2 ablation:
#   - v512 step_delta codebook (xy-only, [V=512, 2])
#   - residual delta head ON   (xy refine on top of codebook)
#   - heading refine head ON   (codebook has no heading → head is the only source, no override issue)
...
echo "Starting DiffusionDrive-AR v2 (v512 step_delta + heading refine + full conditioning)..."
echo "Codebook    : codebook_cache/navsim_kdisk_v512/ego.npy  (V=512, [V,2] xy-only)"
echo "Mode        : step_delta"
echo "Refinement  : residual delta ON, heading head ON (no override risk in step_delta)"
...
python -m navsim.planning.script.run_training \
    ...
    +experiment_name=diffusiondrive_ar_v512_full_v2 \
    ...
    output_dir=/data2/byounggun/diffusiondrive_ar_output/v512_full_v2 \
    wandb.enabled=true \
    wandb.project="diffusiondrive-ar" \
    wandb.name="diffusiondrive_ar_v512_full_v2"
