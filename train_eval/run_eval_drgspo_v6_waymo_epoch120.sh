#!/bin/bash
# Evaluate DiffusionDrive-AR Dr.GSPO v6_waymo epoch-120 fine-tuned checkpoints.
# Checkpoints: grpo-epoch=NN.ckpt (0-indexed, epochs 0-19 for max_epochs=20)
# Defaults to epochs 15-19 + last.ckpt. Override START_EPOCH / END_EPOCH if needed.
...
CKPT_DIR="${CKPT_DIR:-/data2/byounggun/diffusiondrive_dr_gspo_output_v6_waymo_epoch120/checkpoints}"
EVAL_ROOT="${EVAL_ROOT:-/data2/byounggun/diffusiondrive_dr_gspo_output_v6_waymo_epoch120/eval_milestones}"
METRIC_CACHE_PATH="${METRIC_CACHE_PATH:-/data2/byounggun/metric_cache}"
START_EPOCH="${START_EPOCH:-15}"
END_EPOCH="${END_EPOCH:-19}"
...
echo "=================================================="
echo "Evaluating Dr.GSPO v6_waymo epoch-120 fine-tuning"
...
run_eval_one() {
    ...
    local hydra_ckpt="/tmp/drgspo_v6_waymo_epoch120_epoch_${epoch_tag}.ckpt"
    ...
    python3 -m navsim.planning.script.run_pdm_score \
        ...
        experiment_name="drgspo_v6_waymo_epoch120_eval_epoch_${epoch_tag}" \
        output_dir="$out_dir"
...
# Evaluate last.ckpt on the first GPU
if [ -f "$CKPT_DIR/last.ckpt" ]; then
    gpu="${GPU_LIST[0]}"
    local_ckpt="$CKPT_DIR/last.ckpt"
    out_dir="$EVAL_ROOT/epoch_last"
    hydra_ckpt="/tmp/drgspo_v6_waymo_epoch120_epoch_last.ckpt"
    epoch_summary="$EVAL_ROOT/summary_epoch_last.csv"

    ln -sfn "$local_ckpt" "$hydra_ckpt"

    echo "--------------------------------------------------"
    echo "[last] Evaluating"
    echo "Source ckpt : $local_ckpt"
    echo "Hydra alias : $hydra_ckpt"
    echo "GPU         : $gpu"
    echo "Output      : $out_dir"
    echo "--------------------------------------------------"
    ...
    python3 -m navsim.planning.script.run_pdm_score \
        ...
        worker.threads_per_node="$WORKER_THREADS" \
        metric_cache_path="$METRIC_CACHE_PATH" \
        experiment_name="drgspo_v6_waymo_epoch120_eval_epoch_last" \
        output_dir="$out_dir"

    latest_csv="$(find "$out_dir" -maxdepth 1 -type f -name '*.csv' -printf '%T@ %p\n' | sort -n | tail -1 | cut -d' ' -f2-)"
    if [ -n "$latest_csv" ]; then
        python3 - "last" "$local_ckpt" "$out_dir" "$latest_csv" "$epoch_summary" <<'PY'
import csv
import sys
import pandas as pd
