#!/bin/bash
# Dr. GRPO (arXiv:2503.20783) fine-tuning on top of the v9 SFT checkpoint
# (milestone_epoch_110.ckpt, navtest PDMS=0.80241).
#
# v9 vs v6 SFT (architecture differences carried over to RL):
#   - ar_attn_stack_ordering = bev_first  (BEV → Agent → SelfAttn → Ego → FFN)
#   - ar_step_aware_agent     = false      (single-call agent K/V, shared across T)
#   - Otherwise identical: deformable BEV ON, ego cross-attn ON, BEV pos enc ON,
#     agent_topk=30, codebook=navsim_kdisk_v2048_diffusiondrive (BASE).
#
# DR-GRPO recipe (NoRD NAVSIM, applied uniformly across all 4 RL algos):
#   LR = 5e-6 (constant), KL = 0, temperature = 1.0,
#   batch = 128 trajectories / opt step (4 GPUs × 1 × accum=4 × group=8),
#   token-level PPO clip ε = 0.2.
...
SAFE_CKPT="/tmp/drgrpo_v9_base_$(date +%s).ckpt"
ln -sfn "$BASE_CKPT" "$SAFE_CKPT"

# ── Tunables (DR-GRPO / NoRD recipe defaults; override via env) ───────────
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_drgrpo_output_v9}"

EFFECTIVE_BATCH=$(( DEVICES * ACCUMULATE_GRAD_BATCHES * GROUP_SIZE ))

echo "=================================================="
echo "DiffusionDrive-AR Dr. GRPO Fine-tuning  (v9 ep110 base, NoRD recipe)"
echo "=================================================="
...
echo "Clip eps     : $CLIP_EPS     (token-level PPO)"
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_drgrpo_v9 \
    ...
    wandb.name="drgrpo_v9_nord_g${GROUP_SIZE}_t${TEMPERATURE}_lr${LR}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"

echo "=================================================="
echo "Dr. GRPO (v9 base) Training Complete!  Output: $OUTPUT_DIR"
echo "=================================================="
