#!/bin/bash
# Dr. GRPO+ v2 waymo epoch-120 fine-tuning on a random scene_token subset
# from trainval, with the same count as the val-split assignment-triage export.
...
export RANDOM_SCENE_DATA_SPLIT="${RANDOM_SCENE_DATA_SPLIT:-val}"
...
export OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_drgrpo_plus_output_v6_waymo_epoch120_ver2_val_random_scenes}"
...
echo "Random token cache   : $SCRIPT_DIR/.cache/random_scene_tokens_${RANDOM_SCENE_DATA_SPLIT}_seed${RANDOM_SEED}_n${RANDOM_SCENE_COUNT}.json"
...
exec "$SCRIPT_DIR/run_drgrpo_plus_training_v6_waymo_ver2_train.sh" \
    ...
    ++experiment_name=diffusiondrive_ar_drgrpo_plus_v6_waymo_epoch120_ver2_val_random_scenes \
    "wandb.name=drgrpo_plus_v6_waymo_ep120_ver2_val_random_scenes_g${GROUP_SIZE:-12}_lr${LR:-1e-5}_clip${CLIP_EPS:-0.25}_acc${ACCUMULATE_GRAD_BATCHES:-8}" \
    "$@"
