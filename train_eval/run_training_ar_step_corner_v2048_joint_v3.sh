#!/bin/bash
# Train DiffusionDrive-AR — JOINT trunk training v3 (BEV pos-enc ablation):
#   - based on joint_v2 (v2048 step_corners + heading head ON + step-aware agent + ego cross-attn)
...
# Hypothesis: flat BEV gives richer K/V bandwidth than deformable(num_points=1);
# the explicit 2D pos enc compensates for the lost spatial localization.
...
echo "Starting DiffusionDrive-AR step-corner v2048 JOINT v3 (flat BEV + 2D pos enc)..."
...
python -m navsim.planning.script.run_training \
    ...
    +experiment_name=diffusiondrive_ar_step_corner_v2048_joint_v3 \
    ...
    output_dir=/data2/byounggun/diffusiondrive_ar_output/step_corner_v2048_joint_v3 \
    wandb.enabled=true \
    wandb.project="diffusiondrive-ar" \
    wandb.name="diffusiondrive_ar_step_corner_v2048_joint_v3" \
    "$@"
