#!/bin/bash
# Train DiffusionDrive-AR — JOINT trunk training v4 (ego cross-attn ablation):
#   - based on joint_v3 (v2048 step_corners + flat BEV + 2D pos enc)
#   - v4 change: per-layer ego cross-attn → ON
...
# Hypothesis: restoring ego-query conditioning on top of v3's flat BEV + 2D
# positional encoding improves planning context without using deformable BEV.
...
echo "Starting DiffusionDrive-AR step-corner v2048 JOINT v4 (flat BEV + 2D pos enc + ego cross-attn)..."
...
echo "BEV pos enc : 2D sin-cos ON (NEW)"
...
python -m navsim.planning.script.run_training \
    ...
    +experiment_name=diffusiondrive_ar_step_corner_v2048_joint_v4 \
    ...
    output_dir=/data2/byounggun/diffusiondrive_ar_output/step_corner_v2048_joint_v4 \
    wandb.enabled=true \
    wandb.project="diffusiondrive-ar" \
    wandb.name="diffusiondrive_ar_step_corner_v2048_joint_v4" \
    "$@"
