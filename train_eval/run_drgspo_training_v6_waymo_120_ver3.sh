#!/bin/bash
# Dr. GSPO v3 (ULTRA AGGRESSIVE + Smart SFT Anchor) on v6_waymo epoch-120
...
SAFE_CKPT="/tmp/drgspo_v6_waymo_epoch120_ver3_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_dr_gspo_output_v6_waymo_epoch120_ver3}"
...
if [ -n "$RESUME_CKPT" ]; then
    ...
    RESUME_SAFE="/tmp/drgspo_v6_waymo_ver3_resume_$(date +%s).ckpt"
...
echo "=================================================="
echo "Dr. GSPO v3 ULTRA (Smart SFT Anchor) - v6_waymo ep120"
echo "=================================================="
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_drgspo_v6_waymo_epoch120_ver3 \
    ...
    wandb.name="drgspo_v6_waymo_ver3_g${GROUP_SIZE}_lr${LR}_sft${SFT_AUX_COEF}" \
    "$@"

echo "Dr. GSPO v3 ULTRA complete. Output: $OUTPUT_DIR"
