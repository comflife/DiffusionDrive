#!/bin/bash
# Evaluate the highest grpo-epoch checkpoint for v6_waymo epoch-120 RL runs on TRAIN split.
#
# This is intended to be called by per-experiment wrappers, but can also be used
# directly:
#   RL_NAME=drgrpo RL_DIR=/path/to/output bash train_eval/run_eval_rl_v6_waymo_epoch120_train_latest.sh
#   EPOCH_TAG=25 RL_DIR=/path/to/output bash train_eval/run_eval_rl_v6_waymo_epoch120_train_latest.sh
...
EVAL_ROOT="${EVAL_ROOT:-$RL_DIR/eval_train_epoch_${OUT_TAG}}"
SUMMARY_CSV="$EVAL_ROOT/summary_pdms.csv"
SAFE_CKPT="/tmp/${RL_NAME}_v6_waymo_epoch120_train_epoch_${OUT_TAG}_$(date +%s).ckpt"
EXP_NAME="${RL_NAME}_v6_waymo_epoch120_train_eval_epoch_${OUT_TAG}"
...
echo "=================================================="
echo "Evaluating $EVAL_LABEL latest/highest checkpoint on TRAIN split"
