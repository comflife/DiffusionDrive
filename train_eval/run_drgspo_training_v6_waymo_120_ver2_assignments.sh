#!/bin/bash
# Dr. GSPO v2 waymo epoch-120 fine-tuning, restricted to scene_token entries
...
export OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_dr_gspo_output_v6_waymo_epoch120_ver2_assignment_scenes}"
...
exec "$SCRIPT_DIR/run_drgspo_training_v6_waymo_120_ver2.sh" \
    ...
    ++experiment_name=diffusiondrive_ar_drgspo_v6_waymo_epoch120_ver2_assignment_scenes \
    "wandb.name=drgspo_v6_waymo_ep120_ver2_assignment_scenes_g${GROUP_SIZE:-12}_lr${LR:-1e-5}_clip${CLIP_EPS:-0.25}_acc${ACCUMULATE_GRAD_BATCHES:-8}" \
    "$@"
