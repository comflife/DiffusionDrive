#!/bin/bash
# GRPO fine-tuning on top of the v9 SFT checkpoint (milestone_epoch_110.ckpt,
# navtest PDMS=0.80241).
#
...
SAFE_CKPT="/tmp/grpo_v9_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_grpo_output_v9}"
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_grpo_v9 \
    ...
    wandb.name="grpo_v9_nord_g${GROUP_SIZE}_t${TEMPERATURE}_lr${LR}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"
