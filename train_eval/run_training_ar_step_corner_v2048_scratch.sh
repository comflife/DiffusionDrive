#!/bin/bash
# Train DiffusionDrive-AR FROM SCRATCH (no pretrained backbone load).
#   - Same architecture as joint_v2 (v2048 step_corners + full conditioning)
#   - But agent.checkpoint_path=null → all weights random init
#   - trunk_lr_mult=1.0 (no pretrained to protect; full lr everywhere)
#   - max_epochs increased — scratch needs more time to converge.
...
echo "Starting DiffusionDrive-AR step-corner v2048 SCRATCH (no pretrained load)..."
...
echo "Trunk       : SCRATCH (no pretrained), full lr 2e-4"
echo "Init        : all weights random"
...
python -m navsim.planning.script.run_training \
    ...
    +experiment_name=diffusiondrive_ar_step_corner_v2048_scratch \
    ...
    agent.config.trunk_lr_mult=1.0 \
    agent.config.freeze_pretrained_trunk=false \
    output_dir=/data2/byounggun/diffusiondrive_ar_output/step_corner_v2048_scratch \
    wandb.enabled=true \
    wandb.project="diffusiondrive-ar" \
    wandb.name="diffusiondrive_ar_step_corner_v2048_scratch"
