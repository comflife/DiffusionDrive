#!/bin/bash
# Evaluate the v2 FROZEN-trunk AR model.
# Default ckpt: latest epoch_NN.ckpt under v512_full_v2_frozen/checkpoints/.
# Override: AR_CKPT=/path/to/specific.ckpt bash $0
...
CKPT_DIR=/data2/byounggun/diffusiondrive_ar_output/v512_full_v2_frozen/checkpoints
AR_CKPT="${AR_CKPT:-$(ls -1 $CKPT_DIR/epoch_*.ckpt 2>/dev/null | sort | tail -1)}"

if [ -z "$AR_CKPT" ] || [ ! -f "$AR_CKPT" ]; then
    echo "Checkpoint not found at: ${AR_CKPT:-$CKPT_DIR/epoch_*.ckpt}"
    echo "Available checkpoints:"
    ls "$CKPT_DIR" 2>/dev/null
    echo "Override with: AR_CKPT=/path/to/ckpt bash $0"
    exit 1
fi

HYDRA_CKPT=/tmp/diffusiondrive_ar_v512_full_v2_frozen_eval.ckpt
ln -sfn "$AR_CKPT" "$HYDRA_CKPT"

echo "=================================================="
echo "Evaluating DiffusionDrive-AR v2 FROZEN (v512)"
echo "Source ckpt : $AR_CKPT"
echo "Hydra alias : $HYDRA_CKPT"
echo "GPU         : $CUDA_VISIBLE_DEVICES"
echo "Output      : /data2/byounggun/diffusiondrive_ar_output/eval_v512_full_v2_frozen_latest"
echo "=================================================="
...
python3 -m navsim.planning.script.run_pdm_score \
    ...
    experiment_name=diffusiondrive_ar_v512_full_v2_frozen_eval \
    output_dir=/data2/byounggun/diffusiondrive_ar_output/eval_v512_full_v2_frozen_latest
