#!/bin/bash
# Dr. GRPO (arXiv:2503.20783) fine-tuning on top of the v5 SFT checkpoint,
# using the NoRD NAVSIM recipe (arxiv 2602.21172, §5.2 + §11.2).
#
# Why NoRD-style: vanilla GRPO with our prior recipe (lr=1e-6, KL=0.05,
# batch=4, T=0.7) produced KL≈0 over 200 steps → policy did not move.
# NoRD's reported NAVSIM run uses much larger gradient steps and no KL
# trust region, which is what actually moves a saturated SFT policy.
#
#   NoRD NAVSIM Dr. GRPO (their numbers):
#     LR = 5e-6 (constant), KL = 0, temperature = 1.0,
#     batch = 128 trajectories / optimizer step, group = 8, ~160 steps total
#
...
#                  = 128 trajectories / optimizer step  (matches NoRD)
#     1 epoch on navtest ≈ 3037 batches / 4 accum ≈ 760 optimizer steps
#     save every 60 optimizer steps → ~12 ckpts per epoch
#
# Reward stays at PDMS only — NoRD's length/format reward terms exist
# because they output a token sequence whose length and format can be
# malformed; our AR head emits fixed-length T=8 valid codebook indices,
# so those terms are no-ops here.
#
# v5 ≠ v6: deformable BEV OFF (flat global MHA), 2D BEV pos enc ON.
#
# Usage:
#   ./run_drgrpo_training_v5.sh
#   BASE_CKPT=/path/to/v5_milestone_epoch_080.ckpt ./run_drgrpo_training_v5.sh
#   ACCUMULATE_GRAD_BATCHES=8 ./run_drgrpo_training_v5.sh   # 256 / opt step
#   LR=1e-5 KL_COEF=0.01 ./run_drgrpo_training_v5.sh        # tweak if needed
...
if [ -z "${BASE_CKPT:-}" ]; then
    # `|| true` is required — `ls` exits non-zero on empty glob, which under
    # `set -euo pipefail` would silently kill the script.
    LATEST_MS=$(ls -1 "$DEFAULT_V5_DIR"/milestone_epoch_*.ckpt 2>/dev/null | sort | tail -1 || true)
    if [ -n "$LATEST_MS" ]; then
        BASE_CKPT="$LATEST_MS"
...
SAFE_CKPT="/tmp/drgrpo_v5_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_drgrpo_output_v5}"

EFFECTIVE_BATCH=$(( DEVICES * ACCUMULATE_GRAD_BATCHES * GROUP_SIZE ))

echo "=================================================="
echo "DiffusionDrive-AR Dr. GRPO Fine-tuning  (v5 base, NoRD recipe)"
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_drgrpo_v5 \
    ...
    wandb.name="drgrpo_v5_nord_g${GROUP_SIZE}_t${TEMPERATURE}_lr${LR}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"

echo "=================================================="
echo "Dr. GRPO Training Complete (v5)! Output: $OUTPUT_DIR"
echo "=================================================="
