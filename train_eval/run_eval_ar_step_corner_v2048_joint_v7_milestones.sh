#!/bin/bash
# Evaluate DiffusionDrive-AR joint_v7 milestone checkpoints and print PDMS.
# v7 = v6 + ar_step_aware_agent OFF (single-call agent attention, no step_emb on K/V).
#       Same warm-start (88.1 PDMS diff_decoder.* -> AR head) and same other
#       conditioning flags (ego cross-attn ON, deformable BEV ON, BEV pos enc ON).
...
TEMP="${TEMP:-0.0}"
# Auto-suffix EVAL_ROOT when sampling (TEMP > 0) so each temperature gets its
# own results dir and doesn't clobber the greedy (temp=0) results.
TEMP_SUFFIX=""
case "$TEMP" in
    0.0|0|0.00) ;;
    *) TEMP_SUFFIX="_temp${TEMP}" ;;
esac

CKPT_DIR="${CKPT_DIR:-/data2/byounggun/diffusiondrive_ar_output/step_corner_v2048_joint_v7/checkpoints}"
EVAL_ROOT="${EVAL_ROOT:-/data2/byounggun/diffusiondrive_ar_output/eval_step_corner_v2048_joint_v7_milestones${TEMP_SUFFIX}}"
...
echo "=================================================="
echo "Evaluating DiffusionDrive-AR joint_v7 milestones"
...
echo "Temperature    : $TEMP  ($([ "$TEMP" = "0.0" ] && echo greedy || echo stochastic-multinomial))"
echo "=================================================="
...
run_eval_one() {
    ...
    local hydra_ckpt="/tmp/diffusiondrive_ar_step_corner_v2048_joint_v7_epoch_${epoch_tag}.ckpt"
    ...
    python3 -m navsim.planning.script.run_pdm_score \
        ...
        experiment_name="diffusiondrive_ar_step_corner_v2048_joint_v7_eval_epoch_${epoch_tag}" \
        output_dir="$out_dir"
