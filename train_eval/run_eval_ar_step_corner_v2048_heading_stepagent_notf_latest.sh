#!/bin/bash
# Evaluate the latest TF=false AR (heading head + step-aware agent) checkpoint.
# Default: 8rmgyc9n/last.ckpt — override AR_CKPT to pick another epoch.
...
AR_CKPT="${AR_CKPT:-/data2/byounggun/diffusiondrive_ar_output/diffusiondrive-ar/8rmgyc9n/checkpoints/last.ckpt}"

if [ ! -f "$AR_CKPT" ]; then
    echo "Checkpoint not found: $AR_CKPT"
    echo "Available checkpoints in 8rmgyc9n:"
    ls /data2/byounggun/diffusiondrive_ar_output/diffusiondrive-ar/8rmgyc9n/checkpoints/ 2>/dev/null
    echo "Override with: AR_CKPT=/path/to/ckpt bash $0"
    exit 1
fi

# Hydra-safe checkpoint alias (avoid '=' grammar issues in epoch=NN filenames)
HYDRA_CKPT=/tmp/diffusiondrive_ar_step_corner_v2048_heading_stepagent_notf_eval.ckpt
ln -sfn "$AR_CKPT" "$HYDRA_CKPT"

echo "=================================================="
echo "Evaluating DiffusionDrive-AR (heading + step-aware agent, TF=false train)"
echo "Source ckpt : $AR_CKPT"
echo "Hydra alias : $HYDRA_CKPT"
echo "GPU         : $CUDA_VISIBLE_DEVICES"
echo "Output      : /data2/byounggun/diffusiondrive_ar_output/eval_heading_stepagent_notf_latest"
echo "=================================================="
...
python3 -m navsim.planning.script.run_pdm_score \
    ...
    experiment_name=diffusiondrive_ar_step_corner_v2048_heading_stepagent_notf_eval \
    output_dir=/data2/byounggun/diffusiondrive_ar_output/eval_heading_stepagent_notf_latest
