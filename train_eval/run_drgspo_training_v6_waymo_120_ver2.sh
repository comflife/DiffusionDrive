#!/bin/bash
# Dr. GSPO v2 (Aggressive) fine-tuning on v6_waymo epoch-120 SFT.
#
# Changes from v1 (NoRD conservative):
#   - LR 5e-6 → 1e-5
#   - GROUP_SIZE 8 → 12
#   - CLIP_EPS 0.2 → 0.25   (token-level for Dr.GSPO)
#   - ACCUMULATE_GRAD_BATCHES 4 → 8
#   - MAX_EPOCHS 40 → 60
#
# Usage:
#   bash train_eval/run_drgspo_training_v6_waymo_120_ver2.sh
#   DEVICES=2 ACCUMULATE_GRAD_BATCHES=8 bash train_eval/run_drgspo_training_v6_waymo_120_ver2.sh
...
SAFE_CKPT="/tmp/drgspo_v6_waymo_epoch120_ver2_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_dr_gspo_output_v6_waymo_epoch120_ver2}"
...
if [ -n "$RESUME_CKPT" ]; then
    ...
    RESUME_SAFE_CKPT="/tmp/drgspo_v6_waymo_epoch120_ver2_resume_$(date +%s).ckpt"
...
echo "=================================================="
echo "DiffusionDrive-AR Dr. GSPO v2 (Aggressive) - v6_waymo epoch-120"
echo "=================================================="
echo "Algorithm    : dr_gspo (v2 aggressive)"
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_drgspo_v6_waymo_epoch120_ver2 \
    ...
    wandb.name="drgspo_v6_waymo_ep120_ver2_g${GROUP_SIZE}_lr${LR}_clip${CLIP_EPS}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"

echo "=================================================="
echo "Dr. GSPO v2 Training Complete! Output: $OUTPUT_DIR"
echo "=================================================="
