...
#
# This is the non-Dr. counterpart for the v6_waymo RL runs. Defaults mirror the
# plain Dr. GRPO Waymo recipe (not ver2/ver3), with a 10 epoch cap.
#
# Usage:
#   bash train_eval/run_grpo_training_v6_waymo_120_10ep.sh
#   BASE_CKPT=/path/to/ckpt.ckpt bash train_eval/run_grpo_training_v6_waymo_120_10ep.sh
#   DEVICES=2 MAX_EPOCHS=10 bash train_eval/run_grpo_training_v6_waymo_120_10ep.sh
...
SAFE_CKPT="/tmp/grpo_v6_waymo_epoch120_10ep_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_grpo_output_v6_waymo_epoch120_10ep}"
...
if [ -n "$RESUME_CKPT" ]; then
    ...
    RESUME_SAFE_CKPT="/tmp/grpo_v6_waymo_epoch120_10ep_resume_$(date +%s).ckpt"
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_grpo_v6_waymo_epoch120_10ep \
    ...
    wandb.name="grpo_v6_waymo_ep120_10ep_g${GROUP_SIZE}_t${TEMPERATURE}_lr${LR}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"
...
bash "$SCRIPT_DIR/run_eval_grpo_v6_waymo_epoch120_10ep.sh"
