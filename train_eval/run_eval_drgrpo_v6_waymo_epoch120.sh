#!/bin/bash
# Evaluate DiffusionDrive-AR Dr.GRPO v6_waymo epoch-120 fine-tuned checkpoints.
...
CKPT_DIR="${CKPT_DIR:-/data2/byounggun/diffusiondrive_drgrpo_output_v6_waymo_epoch120/checkpoints}"
EVAL_ROOT="${EVAL_ROOT:-/data2/byounggun/diffusiondrive_drgrpo_output_v6_waymo_epoch120/eval_milestones}"
...
echo "=================================================="
echo "Evaluating Dr.GRPO v6_waymo epoch-120 fine-tuning"
...
run_eval_one() {
    ...
    local hydra_ckpt="/tmp/drgrpo_v6_waymo_epoch120_epoch_${epoch_tag}.ckpt"
    ...
    python3 -m navsim.planning.script.run_pdm_score \
        ...
        experiment_name="drgrpo_v6_waymo_epoch120_eval_epoch_${epoch_tag}" \
        output_dir="$out_dir"
...
if [ -f "$CKPT_DIR/last.ckpt" ]; then
    ...
    hydra_ckpt="/tmp/drgrpo_v6_waymo_epoch120_epoch_last.ckpt"
    ...
    python3 -m navsim.planning.script.run_pdm_score \
        ...
        worker.threads_per_node="$WORKER_THREADS" \
        metric_cache_path="$METRIC_CACHE_PATH" \
        experiment_name="drgrpo_v6_waymo_epoch120_eval_epoch_last" \
        output_dir="$out_dir"
