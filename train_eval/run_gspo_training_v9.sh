#!/bin/bash
# GSPO (Group Sequence Policy Optimization, Qwen 2025) fine-tuning on top of
# the v9 SFT checkpoint (milestone_epoch_110.ckpt, navtest PDMS=0.80241).
#
# v9 vs v6 architecture (carried over to RL): bev_first attn ordering,
# step_aware_agent=false; otherwise identical (deformable BEV ON, ego cross-attn
# ON, BEV pos enc ON, agent_topk=30, BASE codebook).
#
# Hyperparameters: DR-GRPO / NoRD recipe applied uniformly across all 4 RL algos:
#   LR = 5e-6 (constant), KL = 0, temperature = 1.0,
#   batch = 128 trajectories / opt step (4 GPUs × 1 × accum=4 × group=8).
# GSPO-specific: sequence-level clip ε_seq = 4e-4 (paper range 3e-4 ~ 4e-4).
#
# Usage:
#   ./run_gspo_training_v9.sh
#   ALGORITHM=gspo_token ./run_gspo_training_v9.sh   # token-routed variant
...
SAFE_CKPT="/tmp/gspo_v9_base_$(date +%s).ckpt"
ln -sfn "$BASE_CKPT" "$SAFE_CKPT"

ALGORITHM="${ALGORITHM:-gspo}"
GROUP_SIZE="${GROUP_SIZE:-8}"
KL_COEF="${KL_COEF:-0.0}"
CLIP_EPS_SEQ="${CLIP_EPS_SEQ:-4e-4}"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_${ALGORITHM}_output_v9}"

EFFECTIVE_BATCH=$(( DEVICES * ACCUMULATE_GRAD_BATCHES * GROUP_SIZE ))

echo "=================================================="
echo "DiffusionDrive-AR ${ALGORITHM^^} Fine-tuning  (v9 ep110 base, NoRD recipe)"
echo "=================================================="
...
echo "Clip eps_seq : $CLIP_EPS_SEQ  (GSPO; sequence-level)"
echo "Clip eps     : $CLIP_EPS      (GRPO debug-only)"
echo "LR           : $LR"
echo "Devices      : $DEVICES"
echo "Accum grads  : $ACCUMULATE_GRAD_BATCHES"
echo "Eff. batch   : $EFFECTIVE_BATCH traj/step"
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name="diffusiondrive_ar_${ALGORITHM}_v9" \
    ...
    wandb.name="${ALGORITHM}_v9_nord_g${GROUP_SIZE}_t${TEMPERATURE}_lr${LR}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"

echo "=================================================="
echo "${ALGORITHM^^} (v9 base) Training Complete!  Output: $OUTPUT_DIR"
echo "=================================================="
