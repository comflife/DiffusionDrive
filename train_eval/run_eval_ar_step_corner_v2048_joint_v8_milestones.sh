#!/bin/bash
# Evaluate DiffusionDrive-AR joint_v8 milestone checkpoints and print PDMS.
# v8 = v7 with all extra conditioning OFF:
#       - ar_step_aware_agent  : OFF (single-call agent attention)
#       - ar_use_ego_cross_attn: OFF
#       - ar_use_deformable_bev: OFF (flat MHA over BEV instead)
#       - ar_use_bev_pos_enc   : OFF
#       Same warm-start (88.1 PDMS diff_decoder.* -> AR head) where applicable.
...
CKPT_DIR="${CKPT_DIR:-/data2/byounggun/diffusiondrive_ar_output/step_corner_v2048_joint_v8/checkpoints}"
EVAL_ROOT="${EVAL_ROOT:-/data2/byounggun/diffusiondrive_ar_output/eval_step_corner_v2048_joint_v8_milestones}"
...
echo "=================================================="
echo "Evaluating DiffusionDrive-AR joint_v8 milestones"
...
run_eval_one() {
    ...
    local hydra_ckpt="/tmp/diffusiondrive_ar_step_corner_v2048_joint_v8_epoch_${epoch_tag}.ckpt"
    ...
    python3 -m navsim.planning.script.run_pdm_score \
        ...
        experiment_name="diffusiondrive_ar_step_corner_v2048_joint_v8_eval_epoch_${epoch_tag}" \
        output_dir="$out_dir"
