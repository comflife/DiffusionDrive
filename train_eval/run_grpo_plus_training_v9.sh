#!/bin/bash
# GRPO+ (GSPO sequence-level ratio + group-divergence-weighted per-token
# advantage) fine-tuning on top of the v9 SFT checkpoint
# (milestone_epoch_110.ckpt, navtest PDMS=0.80241).
#
...
# GRPO+ specific: sequence-level clip ε_seq = 4e-4 (GSPO range), α = 0.5
# (blend: 0=pure GSPO, 1=pure token-attention).
...
SAFE_CKPT="/tmp/grpo_plus_v9_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_grpo_plus_output_v9}"

EFFECTIVE_BATCH=$(( DEVICES * ACCUMULATE_GRAD_BATCHES * GROUP_SIZE ))

echo "=================================================="
echo "DiffusionDrive-AR GRPO+ Fine-tuning  (v9 ep110 base, NoRD recipe)"
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name="diffusiondrive_ar_grpo_plus_v9" \
    ...
    wandb.name="grpo_plus_v9_nord_a${ALPHA}_g${GROUP_SIZE}_t${TEMPERATURE}_lr${LR}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"

echo "=================================================="
echo "GRPO+ (v9 base) Training Complete!  Output: $OUTPUT_DIR"
echo "=================================================="
