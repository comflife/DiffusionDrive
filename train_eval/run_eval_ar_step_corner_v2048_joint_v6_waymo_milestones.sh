#!/bin/bash
# Evaluate DiffusionDrive-AR joint_v6_waymo milestone checkpoints and print PDMS.
# This matches run_training_ar_step_corner_v2048_joint_v6_waymo.sh:
#   - v2048 step_corners with the Waymo-derived DiffusionDrive codebook
#   - step-aware agent attention ON
#   - per-layer ego cross-attn ON, deformable BEV ON, BEV pos enc ON
#   - residual delta ON, heading head ON
...
CKPT_DIR="${CKPT_DIR:-/data2/byounggun/diffusiondrive_ar_output/step_corner_v2048_joint_v6_waymo/checkpoints}"
EVAL_ROOT="${EVAL_ROOT:-/data2/byounggun/diffusiondrive_ar_output/eval_step_corner_v2048_joint_v6_waymo_milestones}"
...
echo "=================================================="
echo "Evaluating DiffusionDrive-AR joint_v6_waymo milestones"
...
run_eval_one() {
    ...
    local hydra_ckpt="/tmp/diffusiondrive_ar_step_corner_v2048_joint_v6_waymo_epoch_${epoch_tag}.ckpt"
    ...
    python3 -m navsim.planning.script.run_pdm_score \
        ...
        experiment_name="diffusiondrive_ar_step_corner_v2048_joint_v6_waymo_eval_epoch_${epoch_tag}" \
        output_dir="$out_dir"
