#!/bin/bash
# Evaluate Dr. GRPO (v9 base) ALL epoch checkpoints (epoch=00 .. epoch=09 + last)
...
CKPT_DIR="${CKPT_DIR:-/data2/byounggun/diffusiondrive_drgrpo_output_v9/checkpoints}"
EVAL_ROOT="${EVAL_ROOT:-/data2/byounggun/diffusiondrive_drgrpo_output_v9/eval_all_epochs}"
...
run_eval_one() {
    ...
    local hydra_ckpt="/tmp/drgrpo_v9_epoch_${epoch_tag}.ckpt"
    ...
    python3 -m navsim.planning.script.run_pdm_score \
        ...
        experiment_name="drgrpo_v9_eval_epoch_${epoch_tag}" \
        output_dir="$out_dir"
