#!/bin/bash
# Dr. GRPO+ v2 on test assignment scenes plus the same count of non-overlapping
# random test scenes (2x assignment count total).
...
export OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_drgrpo_plus_output_v6_waymo_epoch120_ver2_test_assign_plus_random_scenes}"
export METRIC_CACHE_PATH="${METRIC_CACHE_PATH:-/data2/byounggun/metric_cache}"
...
exec "$SCRIPT_DIR/run_drgrpo_plus_training_v6_waymo_ver2.sh" \
    "++train_test_split.scene_filter.tokens=$COMBINED_SCENE_TOKENS" \
    ++experiment_name=diffusiondrive_ar_drgrpo_plus_v6_waymo_epoch120_ver2_test_assign_plus_random_scenes \
    "wandb.name=drgrpo_plus_v6_waymo_ep120_ver2_test_assign_plus_random_g${GROUP_SIZE:-12}_lr${LR:-1e-5}_clip${CLIP_EPS:-0.25}_acc${ACCUMULATE_GRAD_BATCHES:-8}" \
    "$@"
