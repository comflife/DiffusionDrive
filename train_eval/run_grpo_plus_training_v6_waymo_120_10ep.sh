#!/bin/bash
# Vanilla GRPO+ fine-tuning on top of the v6_waymo epoch-120 SFT checkpoint.
#
# Non-Dr. GRPO+ = GSPO sequence-level ratio + group-divergence token advantage.
# Defaults mirror the plain Dr. GRPO+ Waymo recipe (not ver2/ver3), with a
# 10 epoch cap. The actual GRPO+ clip is clip_eps_seq, so it is matched to the
# plain Dr. clip scale.
#
# Usage:
#   bash train_eval/run_grpo_plus_training_v6_waymo_120_10ep.sh
#   ALPHA=0.7 DEVICES=2 bash train_eval/run_grpo_plus_training_v6_waymo_120_10ep.sh
...
SAFE_CKPT="/tmp/grpo_plus_v6_waymo_epoch120_10ep_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_grpo_plus_output_v6_waymo_epoch120_10ep}"
...
if [ -n "$RESUME_CKPT" ]; then
    ...
    RESUME_SAFE_CKPT="/tmp/grpo_plus_v6_waymo_epoch120_10ep_resume_$(date +%s).ckpt"
...
echo "=================================================="
echo "DiffusionDrive-AR GRPO+ - v6_waymo epoch-120 (10ep)"
echo "=================================================="
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_grpo_plus_v6_waymo_epoch120_10ep \
    ...
    wandb.name="grpo_plus_v6_waymo_ep120_10ep_a${ALPHA}_g${GROUP_SIZE}_t${TEMPERATURE}_lr${LR}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"
...
bash "$SCRIPT_DIR/run_eval_grpo_plus_v6_waymo_epoch120_10ep.sh"
