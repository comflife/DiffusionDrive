#!/bin/bash
# Dr. GRPO+ v2 (Aggressive) fine-tuning on v6_waymo epoch-120 SFT, FULL navtrain CURATED.
#
# Uses NAVSIM's official navtrain curation (1,192 logs / 103,288 frame tokens) —
# the same data the base model was trained on. Metric cache:
#   /data2/byounggun/custom_cache  (symlink: custom_cache/)
#
# Usage (4 GPUs, default):
#   CUDA_VISIBLE_DEVICES=0,1,2,3 bash train_eval/drgrpoplus_train_train.sh
#   CUDA_VISIBLE_DEVICES=0,1,2,3 bash train_eval/drgrpoplus_train_assign_train.sh
...
SAFE_CKPT="/tmp/drgrpo_plus_v6_waymo_epoch120_ver2_train_base_$(date +%s).ckpt"
...
METRIC_CACHE_PATH="${METRIC_CACHE_PATH:-/data2/byounggun/custom_cache}"
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/trained_model/drgrpoplus_train}"
...
if [ -n "$RESUME_CKPT" ]; then
    ...
    RESUME_SAFE_CKPT="/tmp/drgrpo_plus_v6_waymo_epoch120_ver2_train_resume_$(date +%s).ckpt"
...
echo "=================================================="
echo "DiffusionDrive-AR Dr. GRPO+ v2 (Aggressive) - v6_waymo epoch-120 TRAIN (navtrain curated)"
echo "=================================================="
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_drgrpo_plus_v6_waymo_epoch120_ver2_train_curated \
    ...
    wandb.name="drgrpo_plus_v6_waymo_ep120_ver2_train_curated_g${GROUP_SIZE}_lr${LR}_clip${CLIP_EPS}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"

echo "=================================================="
echo "Dr. GRPO+ v2 TRAIN (navtrain curated) Training Complete! Output: $OUTPUT_DIR"
echo "=================================================="
