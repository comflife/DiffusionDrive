...
#   - Token-level PPO clip ε = 0.2 for both sequence and token-attention terms.
#     The original 4e-4 sequence-level clip is far too small for T=8.
#   - pg_tok uses SUM over T (no length normalisation) matching Dr. GRPO.
#   - pg_seq stays MEAN over G as it is sequence-level.
#   - Sequence-level ratio + per-token attention weight (divergence-based)
#     are kept from GRPO+.
#
...
SAFE_CKPT="/tmp/drgrpo_plus_v9_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_dr_grpo_plus_output_v9}"
...
echo "α (blend)    : $ALPHA   (0=pure seq, 1=pure token-attention)"
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_dr_grpo_plus_v9 \
    ...
    wandb.name="drgrpo_plus_v9_nord_a${ALPHA}_g${GROUP_SIZE}_t${TEMPERATURE}_lr${LR}_acc${ACCUMULATE_GRAD_BATCHES}" \
    "$@"
