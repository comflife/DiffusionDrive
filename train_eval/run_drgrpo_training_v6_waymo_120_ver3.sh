...
#
# Key upgrades for maximum performance:
#   - LR=2e-5 (4x from original NoRD)
#   - GROUP_SIZE=16 (much better advantage estimates)
#   - CLIP_EPS=0.3 (more aggressive policy updates)
#   - ACCUMULATE_GRAD_BATCHES=8 → effective batch 512
#   - MAX_EPOCHS=80
#   - KL_COEF=0.001 (tiny trust region as soft anchor)
#   - sft_aux_coef=0.25 (CRITICAL: auxiliary SFT CE loss prevents forgetting)
#   - TEMPERATURE=0.95 (slightly sharper, higher quality rollouts)
#
# This is the "smart max aggression" recipe.
...
SAFE_CKPT="/tmp/drgrpo_v6_waymo_epoch120_ver3_base_$(date +%s).ckpt"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data2/byounggun/diffusiondrive_drgrpo_output_v6_waymo_epoch120_ver3}"
...
if [ -n "$RESUME_CKPT" ]; then
    ...
    RESUME_SAFE="/tmp/drgrpo_v6_waymo_ver3_resume_$(date +%s).ckpt"
...
echo "=================================================="
echo "LR=$LR  G=$GROUP_SIZE  clip=$CLIP_EPS  acc=$ACCUMULATE_GRAD_BATCHES"
echo "Eff.batch=$EFFECTIVE_BATCH  epochs=$MAX_EPOCHS  sft_aux=$SFT_AUX_COEF"
echo "=================================================="
...
python3 -m navsim.agents.diffusiondrive.grpo_train \
    ...
    ++experiment_name=diffusiondrive_ar_drgrpo_v6_waymo_epoch120_ver3 \
    ...
    wandb.name="drgrpo_v6_waymo_ver3_g${GROUP_SIZE}_lr${LR}_sft${SFT_AUX_COEF}" \
    "$@"
