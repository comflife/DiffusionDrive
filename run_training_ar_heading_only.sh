#!/bin/bash
# Train DiffusionDrive-AR with token-only XY and heading-only refinement.
...
echo "Starting DiffusionDrive-AR heading-only ablation training..."
...
echo "Decoder: discrete AR with token-only XY and learned heading head"
...
python -m navsim.planning.script.run_training \
    ...
    +experiment_name=diffusiondrive_ar_heading_only_v512 \
    ...
    wandb.name="diffusiondrive_ar_heading_only_v512"
